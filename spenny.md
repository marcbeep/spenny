...
- Categories can have can have goals (only one goal per category) that require money to be assigned to them:
  - A user might have a specified "target" to assign by a required day of the week. For example, a user might want to assign £50 to "Groceries" by Sunday every week. We call these `spending` goal.
  ...
  - A user might have a specified "target" to achieve, but no known date. For example, a user might want to save up for a pair of shoes that cost £100. We call these `saving` goal.
  - A user might not have a specified "target" to achieve, and no known date, but might not want the money assigned to a category to fall below a certain amount. For example, the "Emergency Fund" category must always have £1000 assigned. We call these `minimumbalance` goal.
